...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        busqueda = 'Lugares turisticos en san rafael'
        ...
        for result in data['results']:
            place = Place(name=result['name'], address=result['formatted_address'], latitude=result['geometry']['location']['lat'], longitude=result['geometry']['location']['lng'], rating = result['rating'], users_rating = result['user_ratings_total'])
            place.save()
        ...
        try:
            with open('query.json', 'w') as archivo:
